...
namespace PowerString
{
    public partial class ResultForm : Form
    {
        ...
        private string _quote;//클리어 멘트를 위한 string
    ...
    private void ResultForm_Load(object sender, EventArgs e)
        {
            ClearImg.Image.Dispose();
            if (!_perfect)
            {
                ClearImg.Image = ClearImgList.Images[1];//나무늘보 사진으로 교체
                _quote = "부지런하게 연습해봅시다.";//퍼펙트가 아니면 부지런히 코디합시다.
            }
            else
            {
                if (_score < 100)
                {
                    ClearImg.Image = ClearImgList.Images[0];//거북이 사진으로 교체
                    _quote = "성의있는 코딩도 좋지만 조금 더 빠르게 치시면 좋겠어요.";
                    //다 정답이지만 시간이 부족한 경우, 좀 더 노력합시다,
                }
                else
                {
                    ClearImg.Image = ClearImgList.Images[2];//참 잘했어요!
                    _quote = "빠르면서도 완벽한 코딩이었습니다!";//다 정답이고 빨리 쳤습니다!
                }
            }
            String tName = _tester.TesterName;
            TextGroup.Text = $"{tName}님은";
            ClearQuote.Text = $"{_time}초 동안 코드를 작성하셨습니다.{Environment.NewLine}" +
                              $"{Environment.NewLine}" +
